...
The whole workspace runs in the browser: patterns are generated by JavaScript ports of the MxN generators, drawn by [OpenStrandJS](https://github.com/ysetbon/OpenStrandJS) (the browser port of the OpenStrandStudio renderer), with animal markers, strand labels and the Continuation page's alignment search ported too. When the Python renderer is running on your computer (below), the site uses it instead, which adds native Qt rendering and the Jev guided search.
