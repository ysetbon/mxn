...
// Without the local renderer, starting stitches are generated by generators.js,
// drawn by the vendored OpenStrandJS renderer (vendor/strand-renderer.js), and
// markers are painted over them by markers.js.
async function renderInBrowser(s) {
  ...
  const bounds=strandBounds(strands),minX=bounds.x,minY=bounds.y,width=Math.ceil(bounds.width),height=Math.ceil(bounds.height);
  ...
  if(s.animals||s.names){const options={strands:state.data.strands,bounds,k:s.k,hand:s.hand,animals:s.animals,names:s.names,transparent:s.transparent,scale:s.scale,emojiSet:s.emojiSet},layout=computeMarkerLayout(options);
    if(s.animals)await preloadEmojiSet(s.emojiSet,layout.items.map(x=>x.txt));drawMarkers(canvas.getContext('2d'),options,layout);}
...
const render=s=>engine==='native'?request('/api/render',s):renderInBrowser(s);
function useEngine(name){engine=name;connected=true;const browser=name==='browser';
  $('emoji-set').disabled=!$('animals').checked;
