...
- `dist/continuation-engine.js` ports the Continuation workflow's computation (`generate_json` of `mxn_lh_continuation.py`/`mxn_rh_continuation.py`, the k orders and pairs, the angle-range preview, manual pair extensions and the exhaustive CPU `align_level_parallel` search); `dist/alignment-worker.js` runs that search in a module Web Worker, split across nested workers. `continuation.js` uses them whenever the Starting stitch came from the browser engine; the Jev panel is hidden then, since the key must stay on the local renderer. `test_browser_continuation.py` compares documents, pairs, previews and whole `Workflow.run` actions, alignment included, with Python (needs node and numpy; `MXN_PARITY_FULL=1` runs every 1×1–3×3 at k −2…2).
...
- `dist/browser-render.js` draws a document plus markers for both pages. **Reconnect renderer** switches to the native renderer once it is running.
